...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    """           
    # Read dog names from file into a set for fast lookup
    dognames_set = set()
    with open(dogfile, "r") as f:
        for line in f:
            name = line.strip().lower()
            if name != "":
                dognames_set.add(name)

    for key in results_dic:
        # Pet image label
        pet_label = results_dic[key][0]
        # Classifier label can contain multiple names separated by commas
        ...
        for label in classifier_labels:
            if label in dognames_set:
                is_classifier_dog = 1
                break

        # Append results to the value list at indices 3 and 4
